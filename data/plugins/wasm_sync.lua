...
config.plugins.autosync = common.merge({
  ...
  config_spec = {
    ...
    },
    {
      ...
      min = 0.001,
      on_apply = function(v)
        wasm.idbsync_set_interval(v * 1000)
        setup_workspace_save()
      end,
    },
    {
...
function Doc:save(...)
  ...
  if config.plugins.autosync.sync_on_save then
    wasm.idbsync_save_debounced(config.plugins.autosync.save_debounce * 1000)
  end
end
...
function core.load_user_directory()
  ...
  if ok then
    wasm.idbsync_set_interval(config.plugins.autosync.sync_interval * 1000)
    wasm.idbsync_set_auto_sync(config.plugins.autosync.sync_auto)
    setup_workspace_save()
  end
  return ok, err
end
