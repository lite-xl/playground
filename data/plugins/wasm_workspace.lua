...
local function get_workspace_filename(project_dir)
  ...
  local id = #id_list == 0 and 1 or (id_list[#id_list].id + 1)
  local basename = common.basename(project_dir)
...
local function save_workspace()
  ...
  local ok, err = wasm.idbsync_save_sync()
  if ok then
    wasm.idbsync_set_workspace_sync_status("Synced.")
    core.log_quiet("Workspace synced.")
  else
    wasm.idbsync_set_workspace_sync_status("Cannot sync. Please check your console.")
